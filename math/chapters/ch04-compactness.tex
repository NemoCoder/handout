...
\begin{keypoint}[本章要点]
  \begin{itemize}
    ...
    \item 紧性同样有三副面孔：序列紧、全有界加完备、开覆盖紧。
          卷四取开覆盖紧作定义；序列紧在那里仍说得出口，而与之不再普遍等价；
          全有界与完备性要有度量才谈得上。
...
\begin{concept}{紧性}
  \cwhy{连续性本身推不出取到最值。上面两个例子表明，缺的是定义域的一条性质；
    本章要把它找出来并证明它足够。}
  \crel{它是第 1 章确界原理在拓扑层面的接替者。确界原理要先有序结构，
    而度量空间本身不带序；紧性只用开集就能说清，因而走得更远。}
...
\begin{tabular}{@{}lll@{}}
  ...
  全有界加完备（\cref{sec:totally-bounded}） & 最便于检验，可具体估算 & 不由拓扑单独决定 \\
...
\begin{example}[闭且有界而不紧，其一：不完备]
  ...
  \begin{worked}
    \(A\) 在 \(\Q\) 中闭：\(A = [0,\sqrt2\,] \cap \Q\)，而 \([0,\sqrt2\,]\) 在 \(\R\) 中闭，
    由\cref{lem:subspace-openclosed}(ii) 即得。有界显然。
    注意 \(A\) 在 \(\R\) 中并不闭——它的闭包含有 \(\sqrt2\)。
...
\begin{pitfall}
  \begin{enumerate}[label=(\arabic*), leftmargin=1.6em]
    \item \textbf{以为「闭且有界」就是紧。} 上面两个反例说明不然。
          这条等价在欧氏空间中成立（\cref{thm:heine-borel}），
          在一般度量空间中不保证成立。
          （它在别的空间里也可能碰巧成立：\(\Z\) 配通常度量时，
          有界子集都是有限集，闭且有界与紧同样等价。）
    ...
    \item \textbf{把\cref{ex:ball-not-compact} 说成「有限维特有」而不加限定。}
          ...
          （卷五的 Riesz 引理）。这里的「有限维」指向量空间的维数，
          要有线性结构才谈得上，不能不加说明地套到一般度量空间上
          （度量空间另有拓扑维数一类概念，与此不是一回事）。
  \end{enumerate}
\end{pitfall}
...
\begin{proof}
  \(X = \emptyset\) 时空洞成立。否则取 \(\varepsilon=1\) 的有限网
  \(x_{1},\dots,x_{N}\)（因 \(X\) 非空，网非空），
...
\begin{remark}[通往卷四的门]
  ...
  这与\cref{ch:continuity}的情形一致，对照那里的同一条注即可。
...
届时\cref{thm:compact-equiv} 会被拆开，详见\cref{sec:open-cover}末尾的注。
这里只补一条去向：Tychonoff 定理断言任意多个紧空间之积仍紧，本讲义在卷四证明，
用的正是\cref{exr:fip} 的有限交性质形式。

\emph{其二}，\cref{def:totally-bounded} 的全有界在第 9 章与卷五重现。
Arzelà--Ascoli 定理以「有界加等度连续」刻画 \(C(K)\) 中的\emph{相对紧}子集，
即闭包紧者；这里 \(K\) 是紧度量空间，\(C(K)\) 配上确界范数。
要断言集合自身紧，还须加上闭性。
（反例：\(K=[0,1]\) 上全体常值函数 \(f_{c} \equiv c\)，\(0<c<1\)。
该族一致有界且等度连续，但 \(f_{1/n}\)（\(n \ge 2\)）趋于不属于该族的零函数，
故不闭、不紧。）
...
\begin{exercise}[例行]
  ...
  为 \([0,1]\) 构造 \(\varepsilon\)-网并数清所需点数；
  再对 \([0,1]^{n}\) 做同样的事，分别按 \(d_{\infty}\) 与欧氏度量 \(d_{2}\) 算一遍。
  \begin{solution}
    ...
    \([0,1]^{n}\)：取各坐标网格点的乘积
    \(\setst{(j_{1}/N,\dots,j_{n}/N)}{0 \le j_{i} \le N}\)，共 \((N+1)^{n}\) 个点。
    任一 \(x\) 的每个坐标与最近的 \(j_{i}/N\) 相距不超过 \(1/(2N)\)，于是
    \[
      d_{\infty}(x,x_{j}) \le \frac{1}{2N}, \qquad
      d_{2}(x,x_{j}) \le \frac{\sqrt{n}}{2N} .
    \]
    按 \(d_{\infty}\)，取 \(N>1/(2\varepsilon)\) 即可；按 \(d_{2}\)，
    要取 \(N>\sqrt{n}/(2\varepsilon)\)。
    \emph{两者不能混用}：\(n=2\)、\(\varepsilon=0.3\) 时 \(N=2\) 满足前一个不等式，
    但 \((0.25,0.25)\) 到最近网点的欧氏距离约为 \(0.354>\varepsilon\)。
...
\begin{exercise}[例行]
  ...
  \begin{solution}
    \(\arctan\) 的连续性本卷取作已知（初等函数的连续性在第 6 章正式处理）。
    它是 \(\R\) 到 \((-\pi/2,\,\pi/2)\) 上的双射，故 \(\abs{\arctan x} < \pi/2\)
    恒成立，即 \(\pi/2\) 是上界而取不到。
    （双射与连续是两回事，不能由前者推后者，\cref{exr:compact-homeo}
    正是在讨论这个缺口。）
...
\begin{exercise}[结构]
  ...
  \begin{solution}
    取补集即可（\cref{exr:open-iff-complement-closed}）。
    设 \(X\) 紧，\(\mathcal{F}\) 为闭集族且总交为空。
...
\begin{exercise}[结构]
  ...
  \begin{solution}
    ...
    设 \(A \subseteq \R^{n}\)（配欧氏度量）有界，则 \(A \subseteq [-M,M]^{n}\)。
    由\cref{exr:eps-net-count} 的 \(d_{2}\) 一栏（把 \([0,1]^{n}\) 伸缩平移到
    \([-M,M]^{n}\)，网格步长随之乘以 \(2M\)），该立方体对每个 \(\varepsilon>0\)
    都有有限 \(\varepsilon\)-网，其点未必在 \(A\) 中；
