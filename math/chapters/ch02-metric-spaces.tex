...
\begin{remark}[读法建议]
  ...
  要记住的是完备化存在且唯一，技术验证可以回头再看。
  \cref{sec:contraction}的压缩映射原理必须掌握，其证明只有半页，宜熟记。
  \cref{lem:subspace-openclosed} 的相对开闭判据虽只有三行，第 3 至 5 章反复要用。
  习题中\cref{exr:sequential-closure} 的闭包序列刻画也属后续必需，标了「后续必需」的题都该做。
\end{remark}
