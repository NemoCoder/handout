...
这就像拿两张照片作比：问哪一张「更大」没有意义，问两张有多像却有意义。
...
\begin{concept}{度量}
  ...
  \crel{它是绝对值 \(\abs{x-y}\) 的抽象。加上线性结构就得到范数
    （\cref{sec:norm}），再加上角度就得到内积。度量是这一系列结构中最弱的一层，因而适用面最广。}
...
\begin{intuition}
  把一张本市地图缩印之后，随手平摊在本市的地面上。
  \cref{thm:contraction} 断言：地图上必有一点，恰好压在它自己所代表的
  那个实际位置的正上方。而且这样的点只有一个。

  这不是玄谈。缩印就是把实际距离按固定比例缩小，即\cref{eq:contraction}；
  地面是完备的；于是定理适用。要找到那一点也不必碰运气：
  在地图上随便点一处，看它代表哪里，走过去，再在地图上找那个新位置，
