...
第 1 章末尾留下了一个判断：实数系的各条性质中，依赖序的搬不出 \(\R\)，
依赖距离的搬得出去。本章兑现这个判断。
...
同一列函数，在一种距离下发散、在另一种距离下收敛。这说明距离不是集合自带的东西，
而是我们附加上去的结构，且这个结构的选择直接决定了分析学的结论。
把「距离」这一概念本身作为研究对象，正是本章的出发点。

注意在整个讨论中，我们从未问过「\(f_{n}\) 与 \(g\) 谁大谁小」。
在 \(C[0,1]\) 上这个问题没有意义：\(x\) 与 \(x^{2}\) 在 \([0,1]\) 上互有高低。
序在这里不存在，而距离存在。这正是第 1 章所预告的情形。
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 把 \(\R\) 上的收敛、开集、闭集逐条搬过来，看哪些搬得动
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \(C(X)\) 与一致度量 & 第 8 章 & 函数列一致收敛；Stone--Weierstrass \\
    完备性         & 第 8 章 & \(C(X)\) 完备；卷五 \(L^{p}\) 空间 \\
    完备化         & 卷五     & \(L^{p}\) 空间的构造 \\
...
\begin{remark}[读法建议]
  \cref{sec:metric-def} 的例子库必须逐个算过，后面每遇到一个新概念都要回来
  在这些例子上验证一遍，这是本章唯一有效的学法。
  \cref{sec:norm} 关于范数与内积的部分初读可略，它在卷二才真正用上；
  \cref{sec:completion} 完备化的构造细节可以先跳过结论以后再补。
  \cref{sec:contraction} 的压缩映射原理必须掌握，其证明只有半页，值得背下来。
\end{remark}
...
\begin{concept}{度量}
  \cwhy{第 1 章证明了确界原理依赖序而 Cauchy 完备性只依赖距离。
    要把后者用到 \(C[0,1]\) 这类没有序的集合上，先得说清什么叫距离。}
  \crel{它是绝对值 \(\abs{x-y}\) 的抽象。加上线性结构就得到范数
    （\cref{sec:norm}），再加上角度就得到内积。度量是这一串结构里最弱的一层，
    因而适用面最广。}
  \cwhere{分析学与拓扑学的公共底座。泛函分析、微分几何、概率论中的
    Wasserstein 距离、数据科学中的相似性度量，都建立在这三条公理上。}
...
下面把例子一次列足。此后每引入一个新概念，都应回到这批例子上逐一验证；
这是本章唯一有效的学法。
...
\begin{example}[\(\R^{n}\) 上的三个度量]
  ...
  \(d_{1}\) 之所以叫出租车度量，是因为它量的是沿坐标方向行走的路程，
  正如街道呈方格网的城市里出租车所走的距离。
\end{example}
...
\begin{figure}[htbp]
  ...
  \caption{\(\R^{2}\) 上三个度量的单位球 \(\setst{x}{d(x,0) \le 1}\)。
    ...
    这三个度量将在\cref{exr:equiv-metrics}中被证明是等价的，
    即它们给出相同的开集与相同的收敛数列。}
...
\begin{example}[离散度量]
  ...
  三点集上的离散度量可以想成单位正三角形的三个顶点，两两距离为 \(1\)。
\end{example}
...
\begin{example}[连续函数空间与一致度量]
  ...
  这是本讲义中最重要的函数空间例子。第 8 章将证明 \((C[a,b], d_{\infty})\) 完备，
  这个事实是全部逼近理论的基础。
\end{example}
...
\begin{example}[子空间度量]
  ...
  设 \((X,d)\) 为度量空间，\(A \subseteq X\) 非空。把 \(d\) 限制在 \(A \times A\) 上，
...
\begin{pitfall}
  \begin{enumerate}[label=(\arabic*), leftmargin=1.6em]
    \item \textbf{以为度量是集合自带的。} 度量是附加的结构。
...
\begin{remark}
  \label{rem:not-every-metric-from-norm}
...
\begin{remark}
  \label{rem:finite-only}
  (iii) 中的「有限」不能去掉：在 \(\R\) 中 \(\bigcap_{n \ge 1}(-1/n,\,1/n) = \set{0}\)，
  是可数多个开集之交，却不是开集。取最小值的那一步正是有限性用武之处。

  \cref{thm:metric-topology} 的三条性质将在卷四被抽出来作为\term{拓扑空间}{topological space}
...
\begin{example}[完备与不完备]
  ...
  \begin{worked}
    ...
    开区间 \((0,1)\) 以 \(\R\) 的子空间度量不完备：\(a_{n} = 1/n\) 是 Cauchy 列，
    极限 \(0\) 不在 \((0,1)\) 中。
...
\(\Q\) 不完备，而 \(\R\) 完备且 \(\Q\) 在其中稠密。第 1 章附录 A 用 Cauchy 列
把 \(\Q\) 补成 \(\R\)。本节说明那不是 \(\Q\) 特有的手法，而是一般构造。
...
\begin{remark}
  取 \(X = \Q\) 便重新得到 \(\R\)。因此第 1 章附录 A 的康托尔构造，
  是\cref{thm:completion} 的一个特例。这也解释了那里为何要对 Cauchy 列作商：
...
\begin{example}[用压缩映射求平方根]
  ...
  \begin{worked}
    ...
    再证压缩。\(f'(x) = \frac{1}{2}(1 - a/x^{2})\)，在 \(x \ge \sqrt{a}\) 上
    \(0 \le f'(x) < 1/2\)。由中值定理 \(\abs{f(x)-f(y)} \le \frac{1}{2}\abs{x-y}\)，
    故 \(f\) 是压缩常数 \(K=1/2\) 的压缩映射。
...
\begin{table}[htbp]
  \centering
  \caption{度量空间中各概念所依赖的结构层次。上层的性质在下层不再有意义，
    下层的性质在上层自动成立。写定理时应当停在够用的那一层。}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \midrule
    集合            & 无                       & --- \\
    拓扑            & 开集、闭集、闭包、连续     & 卷四：不来自度量的拓扑 \\
    度量            & 收敛、Cauchy 列、完备性、有界 & 卷四：无度量则完备性无意义 \\
    范数（度量 + 线性） & 凸性、算子范数            & 卷五：Banach 空间 \\
    内积（范数 + 角度） & 正交、投影                & 卷五：Hilbert 空间 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\emph{其一}，\cref{thm:metric-topology} 的三条性质在卷四被取作拓扑空间的公理。
届时会看到有些拓扑不来自任何度量，且在那样的空间里数列不足以刻画闭包，
必须换成\term{网}{net}——它可以想成「指标集不必可数的数列」。
...
本章的完备化与第 1 章附录 A 从 \(\Q\) 造 \(\R\) 是同一件事，
读者此时应当回看第 1 章，确认那里的每一步都能在\cref{thm:completion}
的证明中找到对应。
...
\begin{exercise}[结构]
  ...
  \begin{solution}
    ...
    \(a_{n}=1/n\) 在 \(d\) 下是 Cauchy 列，在 \(\rho\) 下不是，
    因为 \(\rho(a_{m},a_{n}) = \abs{\varphi(1/m)-\varphi(1/n)}\) 并不趋于零。
...
\begin{exercise}[延伸]
  ...
  \begin{solution}
    ...
    \((C[0,h], d_{\infty})\) 完备（第 8 章），故 \(T\) 有唯一不动点 \(f\)，
...
\begin{exercise}[延伸]
  ...
  把\cref{thm:completion} 的证明与第 1 章附录 A 中康托尔从 \(\Q\) 构造 \(\R\)
