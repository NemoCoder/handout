...
\begin{keypoint}[本章要点]
  \begin{itemize}
    ...
    \item 只有开集判据的陈述里完全不出现距离，卷四直接取它作为连续的定义。
          序列判据在拓扑空间中\emph{仍说得出口}（收敛是拓扑概念），
          但它与连续不再普遍等价；\(\varepsilon\)-\(\delta\) 则要有度量才写得出来。
    \item 一致连续要求 \(\delta\) 不随点变化。它与连续的差别，
          正是第 2 章「依赖度量」与「依赖拓扑」之别的又一个实例。
    \item 收敛是拓扑概念而 Cauchy 性不是。这一句解释了\cref{thm:complete-not-topological}：
          同胚为什么保持收敛却不保持完备性。
  \end{itemize}
\end{keypoint}
...
其一，函数是分析学的主要研究对象，而连续性是其中最基本的一类限制。
第 1、2 两章处理的是数与点，微分、积分、级数则都是关于函数的运算。
这些运算并非都以连续为前提——Riemann 积分容许有限多个间断点，
函数项级数的定义也不要求各项连续——但连续函数是其中性质最好、
结论最强的一类，因而先处理它。

其二，也是更要紧的一条：\emph{「连续」这个概念本身要经历三次改写}，
而这三次改写恰好演示了本书的主线。中学里的说法是「图像不断开」，
这依赖于能把图像画出来，只对 \(\R\) 上的函数说得通。本章将依次给出三种
精确表述。三者在度量空间中等价，但走出度量空间后境遇不同：
序列判据仍写得出而不再等价于连续，\(\varepsilon\)-\(\delta\) 连写都写不出来，
开集判据则原样成为定义。
...
三者在度量空间中彼此等价。离开度量空间之后命运却不同：\(\varepsilon\)-\(\delta\)
条件因提到距离而无从表述；序列判据仍可表述——收敛在任意拓扑空间中都有定义
（\cref{prop:convergence-topological}）——但不再与连续等价，连续仍蕴含保持序列收敛，
反向一般不成立。只有开集判据始终与连续同义，故卷四取它作为定义。
...
\begin{remark}
  记住这一条：\emph{要判断一个映射是否连续，序列判据通常最省力}。
  凡结论形如「某某映射连续」，先试序列；证不出来再换 \(\varepsilon\)-\(\delta\)。
...
\begin{remark}[两副面孔的分工]
  ...
  谈的是 \(\delta\) 与点的关系，用单个收敛数列无法表述；
  它确有序列判据（\cref{thm:uniform-seq}），但要同时用\emph{两列}点。
\end{remark}

\begin{example}[实际选出一个 \(\delta\)]
  \label{ex:choose-delta}
  设 \(f(x)=x^{2}\)，\(p \in \R\) 固定。给定 \(\varepsilon>0\)，具体地写出一个可用的 \(\delta\)。
  \begin{worked}
    先估计。\(\abs{x^{2}-p^{2}} = \abs{x-p}\,\abs{x+p}\)，
    第一个因子正是我们能控制的量，第二个因子却随 \(x\) 变化，须先把它框住。

    办法是\emph{先给 \(\delta\) 加一个上限}。若已要求 \(\abs{x-p}<1\)，
    则 \(\abs{x} < \abs{p}+1\)，于是
    \begin{equation}
      \label{eq:bound-sum}
      \abs{x+p} \le \abs{x}+\abs{p} < 2\abs{p}+1 .
    \end{equation}
    此时 \(\abs{x^{2}-p^{2}} < \abs{x-p}\,(2\abs{p}+1)\)。
    要它小于 \(\varepsilon\)，只需 \(\abs{x-p} < \varepsilon/(2\abs{p}+1)\)。

    两个要求合并，取
    \begin{equation}
      \label{eq:delta-choice}
      \delta = \min\Bigl\{1,\ \frac{\varepsilon}{2\abs{p}+1}\Bigr\} .
    \end{equation}
    则 \(\abs{x-p}<\delta\) 时两条估计同时成立，故 \(\abs{f(x)-f(p)}<\varepsilon\)。

    两点值得记住。其一，\(\delta\) 依赖 \(p\)：\(p\) 越大 \(\delta\) 越小，
    这正是 \(f\) 连续却不一致连续的迹象（见\cref{exr:x2-not-uniform}）。
    其二，\(\min\) 的手法在这类估计中反复出现——一个约束用来框住
    不好控制的因子，另一个用来达到所需的精度。
  \end{worked}
\end{example}

\begin{example}[在什么范围上一致连续]
  \label{ex:one-over-x-uniform}
  设 \(a>0\)。证明 \(f(x)=1/x\) 在 \([a,\infty)\) 上一致连续。
  \begin{worked}
    对 \(x,y \ge a\)，
    \begin{equation}
      \label{eq:one-over-x-lip}
      \Bigl\lvert\frac{1}{x}-\frac{1}{y}\Bigr\rvert
        = \frac{\abs{y-x}}{xy} \le \frac{\abs{x-y}}{a^{2}} .
    \end{equation}
    取 \(\delta = a^{2}\varepsilon\)，则 \(\abs{x-y}<\delta\) 蕴含
    \(\abs{f(x)-f(y)}<\varepsilon\)。这个 \(\delta\) 与点无关，故一致连续。

    对照\cref{ex:not-uniform}：同一个函数在 \((0,1]\) 上\emph{不}一致连续。
    差别在于分母 \(xy\) 有没有正的下界。把定义域从 \(0\) 处切开一段，
    一致连续性就恢复了。这个现象在第 4 章会得到解释：
    \([a,b]\) 紧而 \((0,1]\) 不紧。
  \end{worked}
\end{example}
...
前两副面孔的\emph{陈述}里都出现了距离：序列判据经由球定义的收敛，
\(\varepsilon\)-\(\delta\) 直接摆出 \(d\) 与 \(\rho\)。
本节给出第三副，它一个距离也不用。
（收敛本身其实由拓扑决定，见\cref{prop:convergence-topological}；
这里说的是本书写下这两副面孔时所用的语言。）
...
\begin{remark}[通往卷四的门]
  ...
  由\cref{thm:metric-topology}，开集族满足三条性质，卷四把那三条取作%
  \term{拓扑}{topology}的公理；相应地，卷四把\cref{thm:open-preimage} 的
  结论取作连续的\emph{定义}。

  三副面孔到卷四的不同命运已在\cref{sec:why-continuity}说过，此处不重复。
...
\cref{thm:complete-not-topological} 至关重要。
...
\begin{remark}[对第 2 章完备性反例的解释]
  ...
  这一句话解释了\cref{thm:complete-not-topological}：那里在 \((0,1)\) 上造了两个度量，
...
\begin{intuition}
  ...
  连续的意思是：在每一处，只要步子迈得够小就能做到；但步子该多小，
  可以随位置而变。一致连续的意思是：存在一个统一的步长，
  在图像的任何地方都够用。

  「图像陡」并不直接决定这件事：\(\sqrt{x}\) 在 \([0,1]\) 上斜率无上限，
  却一致连续（\cref{exr:sqrt-uniform}）。要判定不一致连续，
  得真找出两列点（\cref{thm:uniform-seq}）。
\end{intuition}

\begin{theorem}[一致连续的序列判据]
  \label{thm:uniform-seq}
  \(f \mapsdef X \to Y\) 一致连续，当且仅当：对 \(X\) 中任意两列点
  \(\seq{x_{n}}\) 与 \(\seq{y_{n}}\)，
  \begin{equation}
    \label{eq:uniform-seq}
    d(x_{n},y_{n}) \to 0 \implies \rho\bigl(f(x_{n}),f(y_{n})\bigr) \to 0 .
  \end{equation}
\end{theorem}

\begin{proof}
  \emph{必要性}。设 \(f\) 一致连续，\(d(x_{n},y_{n}) \to 0\)，给定 \(\varepsilon>0\)。
  取\cref{eq:uniform} 中的 \(\delta\)，再取 \(N\) 使 \(n \ge N\) 时
  \(d(x_{n},y_{n})<\delta\)，于是 \(\rho(f(x_{n}),f(y_{n}))<\varepsilon\)。

  \emph{充分性}。设 \(f\) 不一致连续，则存在 \(\varepsilon_{0}>0\)，
  使对每个 \(\delta>0\) 都有一对点违反\cref{eq:uniform}。
  取 \(\delta=1/n\)，得两列点 \(x_{n},y_{n}\) 满足 \(d(x_{n},y_{n})<1/n\)
  而 \(\rho(f(x_{n}),f(y_{n})) \ge \varepsilon_{0}\)。
  则 \(d(x_{n},y_{n}) \to 0\) 而像的距离不趋于零，\cref{eq:uniform-seq} 失效。
\end{proof}

\begin{remark}
  与\cref{def:continuous} 比较：连续用\emph{一列}点（\(x_{n} \to x\)，
  参照点 \(x\) 固定），一致连续用\emph{两列}点（两列彼此靠拢，
  不必各自收敛）。差别正在于有无固定的参照点，这与\cref{prop:convergence-topological} 之后区分收敛与 Cauchy 性的理由完全相同。

  注意\cref{thm:uniform-seq} 中的两列不必收敛。\cref{ex:not-uniform}
  所用的 \(x=1/(2n)\)、\(p=1/n\) 正是这样一对：两者都趋于 \(0\)，
  而 \(0\) 不属于 \((0,1]\)。
\end{remark}
...
\begin{example}[连续但不一致连续]
  ...
  \begin{worked}
    ...
    上面的计算给出了定量的证据：在 \(p=1/n\) 附近要保证输出偏差小于 \(1\)，
    所需的 \(\delta\) 不超过 \(p^{2}\) 的量级，而 \(p\) 可以任意接近 \(0\)，
    故没有统一的 \(\delta\)。

    须注意「函数越来越陡」\emph{不能}单独作为非一致连续的判据。
    \(\sqrt{x}\) 在 \(0\) 附近斜率同样无界，却在 \([0,1]\) 上一致连续
    （\cref{exr:sqrt-uniform}）。斜率无界只说明取不到统一的 Lipschitz 常数，
    推不出非一致连续；要否定一致连续，必须像上面那样算出具体的两列点。
  \end{worked}
\end{example}
...
\begin{example}[Lipschitz 蕴含一致连续]
  ...
  \begin{worked}
    ...
    第 2 章的压缩映射（\cref{def:contraction}）是 \(L<1\) 的 Lipschitz 映射，
...
\begin{remark}
  ...
  证据是现成的：\cref{thm:complete-not-topological} 中的
  \(\varphi(x)=\tan(\pi x-\pi/2)\) 是 \((0,1)\) 到 \(\R\) 的同胚，
  故 \(\varphi\) 与 \(\varphi^{-1}\) 都连续。但 \(\varphi\) 不一致连续：
  取 \(x_{n} = 1-\frac{1}{n}\)、\(y_{n} = 1-\frac{1}{2n}\)，则
  \(\abs{x_{n}-y_{n}} = \frac{1}{2n} \to 0\)。由 \(\tan(\pi/2-t) = \cot t\)，
  \[
    \varphi(x_{n}) = \cot\frac{\pi}{n}, \qquad
    \varphi(y_{n}) = \cot\frac{\pi}{2n} ,
  \]
  而 \(t \to 0^{+}\) 时 \(\cot t \sim 1/t\)，故两者之差约为
  \(2n/\pi - n/\pi = n/\pi \to \infty\)，不趋于零。
  再用\cref{thm:uniform-seq}。而恒等映射一致连续。
  同一个拓扑，一致连续与否可以不同。
\end{remark}
...
\begin{example}
  ...
  \((0,1)\) 与 \(\R\) 同胚（\cref{thm:complete-not-topological} 中的 \(\varphi\) 即为一例），
  但 \(\R\) 完备而 \((0,1)\) 不完备。
  \begin{worked}
...
理由见\cref{sec:open-continuity}末尾的注。这里只补一条去向：届时
「保持序列收敛」不再等价于连续，须把序列换成网，这是收敛阶梯的第三级
（第一级是数列，第二级是度量空间中的数列，见\cref{ch:metric}）。
...
\begin{exercise}[例行]
  ...
  \begin{solution}
    由三角不等式 \(d(x,a) \le d(x,y)+d(y,a)\) 得
    \(d(x,a)-d(y,a) \le d(x,y)\)；交换 \(x\) 与 \(y\) 得反向不等式。
    两式合并即 \(\abs{d(x,a)-d(y,a)} \le d(x,y)\)。这正是 Lipschitz 常数为 \(1\) 的条件，
...
\begin{exercise}[例行]
  \label{exr:x2-not-uniform}
  证明 \(f(x)=x^{2}\) 在 \(\R\) 上连续但不一致连续。
  （提示：用\cref{thm:uniform-seq}，取两列点 \(x_{n}=n+1/n\)、\(y_{n}=n\)。）
  \begin{solution}
    连续性由\cref{ex:choose-delta} 得出。

    非一致连续：取 \(x_{n}=n+1/n\)、\(y_{n}=n\)，则 \(d(x_{n},y_{n})=1/n \to 0\)，
    而 \(x_{n}^{2}-y_{n}^{2} = 2 + 1/n^{2} \ge 2\) 不趋于零。
    由\cref{thm:uniform-seq}，\(f\) 不一致连续。

    与\cref{eq:delta-choice} 对照：那里为每个 \(p\) 选的 \(\delta\) 含因子
    \(1/(2\abs{p}+1)\)，随 \(\abs{p}\) 增大而趋于零。这只说明\emph{那一个}选法
    给不出统一的 \(\delta\)，本身推不出不一致连续——换个选法未必不行。
    真正的理由是上面那两列点。
  \end{solution}
\end{exercise}
...
\begin{exercise}[结构]
  ...
  证明\cref{sec:uniform}末尾一条注的断言：\cref{thm:complete-not-topological} 中的
...
\begin{exercise}[延伸]
  ...
  \begin{solution}
    ...
    一般拓扑空间没有这一条，于是无法从连续性失效造出反例数列。
    在第一可数空间中这一条仍然具备，序列判据依旧有效。

    具体反例：取不可数集 \(X\) 上的\emph{余可数拓扑}（开集为空集与补集可数者）。
    其中收敛的数列必最终为常数，故从 \(X\) 到「同一集合配离散拓扑」的恒等映射
    保持序列收敛；但它不连续，因为离散拓扑中的单点集是开集，其原像不开。
...
\begin{exercise}[延伸]
  ...
  \begin{solution}
    ...
    等距映射把 \(X\) 保距地对应到\emph{像} \(f(X)\) 上，
    故 \(X\) 与 \(f(X)\) 在度量层的性质（完备性、有界）一致；
    但这说不出陪域 \(Y\) 的任何事——\(f(X)\) 可以是 \(Y\) 的一个很小的子集。
    同胚则只保持拓扑层。这一对比是\cref{tab:ch3-structure} 的另一种读法。
  \end{solution}
\end{exercise}
