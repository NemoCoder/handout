...
| 3 并行支柱 | 线性代数（进阶）、抽象代数、一般拓扑、复分析 | IB Topological Spaces、Linear Algebra、Groups Rings and Modules、Complex Analysis | 320~400 | 四门达到 Part IB 考试水平 |
...
阶段 1 与阶段 2 是本讲义的核心产出。阶段 4 之后不再自撰，理由见第七节。
...
［查阅］**Willard, _General Topology_**；**Kelley, _General Topology_**（GTM 27）。
查一般性结论用。
...
| 1 | 实数系 | 有序域公理；有理数域的不完备性；完备性的四种等价刻画（确界、单调有界、闭区间套、Cauchy）；不可数性 | Garling I；Tao I |
...
| B | 附录：一般拓扑速览 | 拓扑空间、子空间与积与商拓扑、分离公理，作为通往卷三的接口 | Sutherland；Munkres |

**三处埋线是本卷区别于传统讲义的关键，写作时不得删改：**
第 6 章把导数定义为线性映射；第 7 章提前引入零测集；
第 4 章明确把三个经典定理归因于紧性。第三条正是剑桥所说的
让旧结论「呈现出新的面貌」的机制。
...
`math/chapters/ch01-real-numbers.tex`（实数系与确界原理）内容与新方案第 1 章重合，
但立场需要调整：确界原理不应作为公理单独供奉，而应作为**完备性四种等价刻画之一**呈现，
证完等价性后立即抽象为度量空间的完备性。已有的 \(\sqrt{2}\) 无理性证明
与「有理数集有空隙」的论述可整段保留，它正是引出完备性的最好动机。
...
**风险三：卷册烂尾。** 缓解：见第七节的边界；每卷独立成书、独立编号、独立发布，
卷一完成即可投入使用。
